...
@register_kernel("cauchy")
def build_affinity_cauchy(
    ...
    # ── Cauchy/rational kernel: 1 / (1 + (d/sigma)^decay), mean of directed ──
    kij = 1.0 / (1.0 + (distances / sig_i) ** decay)  # (n, k_eff)
    kji = 1.0 / (1.0 + (distances / sig_j) ** decay)
    vals = 0.5 * (kij + kji)
